...
*/
   ...
   /*
   ** Remove small noisy spots in a mostly-uniform background, optionally limited to an outer border.
   ** bgColor: the background color to enforce (e.g. WHITE)
   ** tolerance: allowed luminance difference from bgColor to be considered background (0-255)
   ** minNeighborCount: minimum number of neighboring pixels (out of 8) that must be close to bgColor
   ** borderWidth: if > 0, only pixels within borderWidth pixels of any image edge are considered for conversion.
   ** Returns a new PelArray.
   */
      PelArray removeBackgroundNoise(int bgColor, int tolerance, int minNeighborCount)
         {
         return removeBackgroundNoise(bgColor, tolerance, minNeighborCount, 0);
         }

      PelArray removeBackgroundNoise(int bgColor, int tolerance, int minNeighborCount, int borderWidth)
         {
         ...
         // Precompute background luminance
         int bgBlue   =  bgColor        & 0x00FF;
         int bgGreen  = (bgColor >> 8)  & 0x00FF;
         int bgRed    = (bgColor >> 16) & 0x00FF;
         int bgLum = (int)Math.round(0.3 * (double)bgRed + 0.589 * (double)bgGreen + 0.11 * (double)bgBlue);

         // Copy original into target as default
         for (iRow = 0;  iRow < iRowCount; ++iRow)
            for (iCol = 0; iCol < iColCount; ++iCol)
               targetArray[iRow][iCol] = arrayOfPels[iRow][iCol];

         // For each pixel, only consider conversion if within border region (when borderWidth>0)
         for (iRow = 0; iRow < iRowCount; ++iRow)
            {
            for (iCol = 0; iCol < iColCount; ++iCol)
               {
               boolean inBorder = (borderWidth > 0) &&
                                 (iRow < borderWidth || iRow >= iRowCount - borderWidth ||
                                  iCol < borderWidth || iCol >= iColCount - borderWidth);

               int pel = arrayOfPels[iRow][iCol];
               int blue   =  pel        & 0x00FF;
               int green  = (pel >> 8)  & 0x00FF;
               int red    = (pel >> 16) & 0x00FF;
               int lum = (int)Math.round(0.3 * (double)red + 0.589 * (double)green + 0.11 * (double)blue);

               // If we're restricting to an outer border, skip all interior pixels entirely
               if (borderWidth > 0 && !inBorder)
                  {
                  continue;
                  }

               if (Math.abs(lum - bgLum) <= tolerance)
                  {
                  // This pixel is already close to background; make it exact background to remove small variations
                  targetArray[iRow][iCol] = bgColor;
                  continue;
                  }
               ...
               for (nRow = iRow - 1; nRow <= iRow + 1; ++nRow)
                  {
                  for (nCol = iCol - 1; nCol <= iCol + 1; ++nCol)
                     {
                     if (nRow == iRow && nCol == iCol) continue;
                     if (nRow < 0 || nRow >= iRowCount || nCol < 0 || nCol >= iColCount) continue;
                     ++availableNeighbors;
                     int npel = arrayOfPels[nRow][nCol];
                     int nblue   =  npel        & 0x00FF;
                     int ngreen  = (npel >> 8)  & 0x00FF;
                     int nred    = (npel >> 16) & 0x00FF;
                     int nlum = (int)Math.round(0.3 * (double)nred + 0.589 * (double)ngreen + 0.11 * (double)nblue);
                     if (Math.abs(nlum - bgLum) <= tolerance) ++neighborBGCount;
                     }
                  }

               int required = Math.min(minNeighborCount, availableNeighbors > 0 ? availableNeighbors : minNeighborCount);
               if (neighborBGCount >= required)
                  targetArray[iRow][iCol] = bgColor;
               }
            }
      ...
      /*
      ** Remove small noisy spots but only within a circular radius from any image corner.
      ** bgColor: background color to enforce
      ** tolerance: luminance tolerance (0-255)
      ** minNeighborCount: minimum neighbor count (uses available neighbors at edges)
      ** cornerRadius: radius in pixels from each corner; only pixels within this radius are considered
      */
         PelArray removeBackgroundNoiseCorners(int bgColor, int tolerance, int minNeighborCount, int cornerRadius)
            {
            int iRow, iCol, nRow, nCol;
            ...
            int rSq = cornerRadius * cornerRadius;

            for (iRow = 0; iRow < iRowCount; ++iRow)
               {
               for (iCol = 0; iCol < iColCount; ++iCol)
                  {
                  // check whether this pixel lies within cornerRadius of any corner (euclidean distance squared)
                  boolean inAnyCorner = false;
                  int dr, dc, distSq;

                  dr = iRow - r0; dc = iCol - c0; distSq = dr*dr + dc*dc; if (distSq <= rSq) inAnyCorner = true;
                  dr = iRow - r1; dc = iCol - c1; distSq = dr*dr + dc*dc; if (distSq <= rSq) inAnyCorner = true;
                  dr = iRow - r2; dc = iCol - c2; distSq = dr*dr + dc*dc; if (distSq <= rSq) inAnyCorner = true;
                  dr = iRow - r3; dc = iCol - c3; distSq = dr*dr + dc*dc; if (distSq <= rSq) inAnyCorner = true;

                  if (!inAnyCorner) continue; // skip pixels not in any corner region
            ...
            return new PelArray(targetArray);
            } // PelArray removeBackgroundNoiseCorners(...)
